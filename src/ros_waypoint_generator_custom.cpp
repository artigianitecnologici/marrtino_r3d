...
public:
  ...
  {
    ros::NodeHandle n("~");
    n.param("dist_th", dist_th_, 6.0); // distance threshold [m]
    n.param("yaw_th", yaw_th_, 180.0*3.1415/180.0); // yaw threshold [rad]
    odom_sub_ = nh_.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose1",
                              1,
                              &WaypointGenerator::addWaypoint, this);
    clicked_sub_ = nh_.subscribe("move_base_simple/goal", 1, &WaypointGenerator::clickedPointCallback, this);
    ...
    reset_sub = nh_.subscribe<std_msgs::Empty>("reset_path",1, &WaypointGenerator::reset_waypoints, this);
  }
  ...
  void reset_waypoints(const std_msgs::Empty::ConstPtr& msg)
  {
  
    ros_waypoint_generator::WaypointArray waypoints2;
    waypoints_ = waypoints2;
    waypoint_box_count_ = 0;
    std::cout << "cleared waypoints";
  ...
  void clickedPointCallback(const geometry_msgs::PoseStamped &point)
  {
    ...
    pose.pose.orientation = point.pose.orientation;
